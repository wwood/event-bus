...
describe Event::Bus do
  ...
  let(:name_resolver) { instance_double('Event::Events::NameResolver') }
  ...
  describe '#notify' do
    ...
    context 'when subscriber is given by symbol (for events in the Event::Events namespace)' do
    ...
    context 'when event is not an instance of event class' do
      ...
      it { expect { bus.notify event_klass }.to raise_error Event::NoEventError }
    end
  end
  ...
  describe '#register' do
    ...
    context 'when malformed name resolver' do
      ...
      it { expect { bus.register(event_klass, proc {}) }.to raise_error Event::EventNameResolveError, %(Transforming "#{event_klass}" into an event name failed for unknown reason.) }
    end
  end
end
